...
int main() {
    ...
    // ── 케이스 1: 순차 주문 ──────────────────────────────
    std::cout << "══ 케이스 1: 순차 주문 (BTC → ETH) ══" << std::endl;

    Order btc;
    btc.symbol          = "BTCUSDT";
    btc.side            = OrderSide::BUY;
    btc.instrument_type = InstrumentType::FUTURES;
    btc.quantity        = 0.5;
    btc.price           = 80000.0;
    executor.executeOrder(btc);
    executor.fillOrder("BTCUSDT"); // 거래소가 체결 통보

    Order eth;
    eth.symbol          = "ETHUSDT";
    eth.side            = OrderSide::BUY;
    eth.instrument_type = InstrumentType::FUTURES;
    eth.quantity        = 2.0;
    eth.price           = 2000.0;
    executor.executeOrder(eth);
    executor.fillOrder("ETHUSDT");

    executor.printPortfolio();

    // ── 케이스 2: 동시 주문 ──────────────────────────────
    std::cout << "\n══ 케이스 2: 동시 주문 (SOL Futures + BTC Options) ══" << std::endl;

    std::vector<Order> multi_orders;

    Order sol;
    sol.symbol          = "SOLUSDT";
    sol.side            = OrderSide::BUY;
    sol.instrument_type = InstrumentType::FUTURES;
    sol.quantity        = 10.0;
    sol.price           = 150.0;

    Order btc_call;
    btc_call.symbol          = "BTCUSDT_OPT";
    btc_call.side            = OrderSide::BUY;
    btc_call.instrument_type = InstrumentType::OPTIONS;
    btc_call.quantity        = 1.0;
    btc_call.price           = 500.0;
    btc_call.option_type     = "CALL";
    btc_call.strike_price    = 100000.0;
    btc_call.expiry          = "20260501";

    multi_orders.push_back(sol);
    multi_orders.push_back(btc_call);
    executor.executeOrders(multi_orders);
    executor.fillOrder("SOLUSDT");
    executor.fillOrder("BTCUSDT_OPT");

    executor.printPortfolio();

    // ── 케이스 3: 매수 미체결 중 매도 주문 ──────────────
    std::cout << "\n══ 케이스 3: 매수 미체결 중 매도 주문 (충돌 처리) ══" << std::endl;

    Order xrp_buy;
    xrp_buy.symbol          = "XRPUSDT";
    xrp_buy.side            = OrderSide::BUY;
    xrp_buy.instrument_type = InstrumentType::FUTURES;
    xrp_buy.quantity        = 100.0;
    xrp_buy.price           = 0.5;
    executor.executeOrder(xrp_buy);
    // ★ 체결 전에 매도 주문 도착!

    Order xrp_sell;
    xrp_sell.symbol          = "XRPUSDT";
    xrp_sell.side            = OrderSide::SELL;
    xrp_sell.instrument_type = InstrumentType::FUTURES;
    xrp_sell.quantity        = 100.0;
    xrp_sell.price           = 0.55;
    executor.executeOrder(xrp_sell); // 충돌 감지 → 매수 취소 → 매도 전송
    executor.fillOrder("XRPUSDT");

    executor.printPortfolio();

    // ── 추가 케이스: 잔고 부족 ───────────────────────────
    std::cout << "\n══ 추가 케이스: 잔고 부족 ══" << std::endl;
    Order big_order;
    big_order.symbol          = "BTCUSDT";
    big_order.side            = OrderSide::BUY;
    big_order.instrument_type = InstrumentType::FUTURES;
    big_order.quantity        = 100.0;
    big_order.price           = 80000.0;
    executor.executeOrder(big_order);

    // ── 추가 케이스: 수량 0 ──────────────────────────────
    std::cout << "\n══ 추가 케이스: 수량 0 ══" << std::endl;
    Order zero_qty;
    zero_qty.symbol          = "ETHUSDT";
    zero_qty.side            = OrderSide::BUY;
    zero_qty.instrument_type = InstrumentType::FUTURES;
    zero_qty.quantity        = 0.0;
    zero_qty.price           = 2000.0;
    executor.executeOrder(zero_qty);

    // ── 추가 케이스: 옵션 필수 정보 누락 ────────────────
    std::cout << "\n══ 추가 케이스: 옵션 필수 정보 누락 ══" << std::endl;
    Order bad_option;
    bad_option.symbol          = "ETHUSDT_OPT";
    bad_option.side            = OrderSide::BUY;
    bad_option.instrument_type = InstrumentType::OPTIONS;
    bad_option.quantity        = 1.0;
    bad_option.price           = 200.0;
    executor.executeOrder(bad_option);

    // ── 추가 케이스: 없는 주문 취소 ─────────────────────
    std::cout << "\n══ 추가 케이스: 없는 주문 취소 시도 ══" << std::endl;
    executor.cancelOrder(9999);
